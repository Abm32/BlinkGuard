 ...
 */
export async function analyzeTransactionSafety(
...
function analyzeBalanceTransfers(balanceChanges: TransactionSimulation['balanceChanges']): BalanceAnalysis {
...
interface ApprovalAnalysis {
  hasSuspiciousApproval: boolean;
  severity: 'low' | 'medium' | 'high' | 'critical';
  description: string;
}

function analyzeApprovals(logs: string[]): ApprovalAnalysis {
...
interface ContractAnalysis {
  hasUnknownContract: boolean;
  contracts: string[];
}

function analyzeContracts(logs: string[], domain: string): ContractAnalysis {
  ...
  const unknownContracts = Array.from(programIds).filter(
    id => !trustedPrograms.includes(id)
  );
...
function analyzeDomainTrust(domain: string): DomainTrust {
  const reasons: string[] = [];
  let isTrusted = true;
